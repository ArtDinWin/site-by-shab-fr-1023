...
sliderData.push(
  new Slider("Sochi, Thieves", 105, 4, "0.5 M", "./images/slider-02.jpg")
);
sliderData.push(
  new Slider("Rostov-On-Don, Patriotic", 93, 3, "1 M", "./images/slider-03.jpg")
);
...
function activateSlider(sliderData) {
  ...
  function renderLinks(sliderActive) {
    ...
    linkSliderList.forEach((linkSlider) => {
      linkSlider.addEventListener("click", (e) => {
        e.preventDefault();
        playAnimation();
  ...
  function initArrows() {
    sliderArrows.forEach((arrow) => {
      arrow.addEventListener("click", function () {
        ...
        console.log("sliderActive: ", nextNumber);
  ...
  function displayActiveSlider(sliderShow) {
    sliderActive = Number(sliderShow);
    displaySliderElements(sliderActive);
    let dots = sliderDots.querySelectorAll(".slider-control__dot");
